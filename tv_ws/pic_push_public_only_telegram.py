...
默认处理 15m / 1h / 4h，信号原文直推 Telegram。

用法:
  python -m tv_ws.pic_push_public_only_telegram
  python -m tv_ws.pic_push_public --only-telegram
  python -m tv_ws.pic_push_public_only_telegram --dry-run --print-raw
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        description="TradingView WSS：15m/1h/4h 信号原文直推 Telegram（不润色、不截图、不发广场）",
    )
