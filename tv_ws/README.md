...
| `pic_push_public_only_telegram.py` | 仅 Telegram：15m/1h/4h 信号原文，不润色、不截图 |
| `pic_push_public_only_telegram_text.py` | 同上（兼容入口） |
...
# 仅 Telegram 原文（15m/1h/4h，不润色、不截图、不发广场）
python -m tv_ws.pic_push_public --only-telegram
python -m tv_ws.pic_push_public_only_telegram
python -m tv_ws.pic_push_public_only_telegram_text  # 兼容入口，行为相同
