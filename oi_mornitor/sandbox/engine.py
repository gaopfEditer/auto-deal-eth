...
class SandboxEngine:
    def __init__(self) -> None:
        ...
        # 同币同已收盘 K 只评估一次（防 60s 扫描反复用同一根 K 假平仓）
    ...
    def _bar_cooldown_sec(self) -> int:
        return _INTERVAL_SEC.get(SANDBOX_INTERVAL, 900) * max(1, SANDBOX_REENTRY_COOLDOWN_BARS)

    def _can_reenter(self, symbol: str, bar_time: int) -> bool:
        sym = symbol.upper()
        last_exit = self._last_exit_bar.get(sym)
        if last_exit is not None and bar_time <= last_exit:
            return False
        if last_exit is not None and (bar_time - last_exit) < self._bar_cooldown_sec():
            return False
        # 持久化成交也检查
        recent = self.tracker.list_trades(symbol=sym, limit=1)
        if recent:
            exit_t = int(recent[0]["exit_time"])
            if bar_time <= exit_t:
                return False
            if (bar_time - exit_t) < self._bar_cooldown_sec():
                return False
        if self._last_entry_bar.get(sym) == bar_time:
            return False
        return True
    ...
    async def manual_enter(
        ...
        *,
        ...
        try:
            ...
            logger.warning("手动开仓指标失败 %s: %s", sym, exc)
        ...
        logger.info("沙盒手动市价入场 %s %s %s @ %s", sym, side_u, logic_u, sig.price)
    ...
    async def scan(
        ...
        *,
        ...
        symbols = sorted(set(pool) | set(open_syms))
        if not symbols:
            self._last_alerts = []
            self._last_scan_ts = scan_ts or time.time()
            return []
        ...
        for sym in symbols:
            ...
            try:
                ...
                logger.warning("沙盒指标失败 %s: %s", sym, exc)
                continue
            # 用已收盘 K（去掉最后一根未完成）
            if len(df) < 61:
                continue
            closed = df.iloc[:-1].copy()
            structure = state_by_sym.get(sym) or {}
            structure = {**structure, "symbol": sym}
            t = bar_time_sec(closed)
            # 同一根已收盘 K 只跑一轮，避免 MACD 弱信号在 0 波动下连环平仓
            if self._last_eval_bar.get(sym) == t:
                continue
            open_positions = self.tracker.list_positions_for_symbol(sym)
            ...
            for pos in open_positions:
                ...
                signals = evaluate_exit_and_trail(
                    ...
                    bar_time=t,
                    interval_sec=_INTERVAL_SEC.get(SANDBOX_INTERVAL, 900),
                    meta=pos_meta,
                )
                current = pos
                for sig in signals:
            ...
            # 自动策略：该币无持仓时才开新仓；手动可同币叠仓（见 manual_enter）
            if (
                not self.tracker.list_positions_for_symbol(sym)
                and sym in pool
                and len(self.tracker.list_positions()) < SANDBOX_MAX_CONCURRENT
                and self._can_reenter(sym, t)
            ):
                sig = evaluate_entry(closed, symbol=sym, structure=structure)
                if sig:
                    alert = self._apply_signal(sym, sig, t, pos=None)
                    if alert:
                        alerts.append(alert)
            self._last_eval_bar[sym] = t
    ...
    def _apply_signal(
        ...
        *,
        ...
        if sig.action == "enter" and pos is None and sig.sl:
            if len(self.tracker.list_positions()) >= SANDBOX_MAX_CONCURRENT:
                return None
            if not force and not self._can_reenter(sym, bar_time):
                return None
            ...
            meta.setdefault("interval", SANDBOX_INTERVAL)
            if not meta.get("ref_intervals"):
                meta["ref_intervals"] = ref_intervals_for_logic(sig.logic)
                meta["ref_intervals_label"] = " · ".join(meta["ref_intervals"])
            source = resolve_entry_source(meta, force_manual=force)
            meta["source"] = source
            ...
            self._last_entry_bar[sym] = bar_time
            if new_pos.id:
                self._last_trail_sl[new_pos.id] = float(sig.sl)
            logger.info(
                "沙盒入场 #%s %s %s %s @ %s SL=%s lev=%.0fx margin=%.2fU",
                new_pos.id,
                sym,
                sig.side,
                sig.logic,
            ...
            return {
                ...
                "status_label": f"沙盒入场 · {src_label} · {label} · {lev:.0f}x",
            }
        ...
        if sig.action == "trail" and pos:
            ...
            return {
                ...
                "interval": SANDBOX_INTERVAL,
                "kline_close_time": bar_time,
                "status_label": f"沙盒移止损 · 逻辑{pos.logic}",
            }
